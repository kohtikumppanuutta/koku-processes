...
function formatDataCache(cache, matrix) {
    if(form1.getCache().getDocument(cache).getFirstChild() == null) {
        form1.getJSXByName(matrix).commitAutoRowSession();
        return true;
    } else {
        return false;
    }
...
function mapSelectedRecipientsToMatrix() {
    ...
    while(childIterator.hasNext()) {
        ...
        if(group != 0) {
            ...
            status = usernamesData.selectSingleNode("//status", "xmlns:ns2='http://soa.tiva.koku.arcusys.fi/'").getValue();
            if(status == "error") {
                error = usernamesData.selectSingleNode("//message", "xmlns:ns2='http://soa.tiva.koku.arcusys.fi/'").getValue();
                alert("Viestin l\u00E4hett\u00E4minen roolille " + roleName + " ep\u00E4onnistui! Virheviesti: " + error);
            } else {
                node = form1.getCache().getDocument("Receipients-nomapNew").getFirstChild().cloneNode();
