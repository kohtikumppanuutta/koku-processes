...
function checkTemplateName() {
    var objXML, nodeIterator, otsikko, node, flag = false;
...
function swapConsentGivers(selection) {
    if (selection == 1) {
        ...
        TIVA_Form.getJSXByName("receipient2-field").setDisplay("none", true);
    } else if (selection == 2) {
        TIVA_Form.getJSXByName("Suostumus_Extend1").setValue("AnyParent");
        TIVA_Form.getJSXByName("receipient2-field").setDisplay("block", true);
    } else if (selection == 3) {
...
function getId()    {
    ...
    TIVA_Form.getJSXByName("id").setValue(parseInt(id) + 1, 10);
    return id;
}
...
function addToRecipients(selection) {
    ...
    var counter, node, hasEmptyChild, chosen, childIterator, uid, firstname, lastname, childNode;
